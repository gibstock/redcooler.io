...
const CategorySelect = ({isFilter, onChange}:AppProps) => {
  const categoryStore = useCategoryStore()
  const savedCategory = categoryStore.savedCategory
  return (
    <div>
      <select required value={savedCategory !== null ? savedCategory : "all"} onChange={onChange} name="categories" id="category-select" className='bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-300 py-2 px-3 rounded-sm'>
