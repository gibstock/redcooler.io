...
const NavBar = () => {
  ...
  return (
    <nav className="relative z-50">
      <div className="sticky-wrapper py-2 flex flex-row min-h-[8vh] w-full justify-between items-center px-4 shadow shadow-slate-300 dark:shadow-none mb-4 fixed top-0 left-0 right-0 bg-white dark:bg-dark-black">
        {user && (
          <>
            <div
              className={`profile-menu flex flex-col justify-evenly items-center absolute transition-all duration-500 top-0 -right-${menuPos} md:w-[40vw] w-[60vw] h-[100vh] bg-white dark:bg-dark-black z-50`}
              style={menuPos === '[40vw]' ? { right: '-60vw' } : { right: '0' }}
            >
              <div
                className="close-menu-icon absolute text-slate-800 dark:text-slate-300 dark:hover:text-slate-600 hover:text-slate-200 top-4 right-4 pr-[2vw] cursor-pointer"
              ...
              <div className="user flex flex-col justify-center items-center gap-4 text-slate-900 dark:text-slate-200">
                <div className="flair flex flex-row justify-center items-center gap-2">
                  <FaQuoteLeft />
                  {userProfile && userProfile[0].flair.length > 0
                    ? userProfile[0].flair
                    : 'noob'}
                  <FaQuoteRight />
                </div>
                <div
                  className="avatar rounded-full relative cursor-pointer hover:opacity-80 active:opacity-50"
                ...
                <div className="dark-mode-group flex flex-col justify-center items-center gap-2">
                  ...
                  <div
                    className="dark-mode-icon border border-blue-200 p-2 rounded-lg cursor-pointer"
            ...
            <div
              className={`overlay absolute transition-all duration-500 top-0 left-0 h-screen w-screen ${
                overlayZ === 'z-40' ? 'opacity-60' : 'opacity-0'
              } ${
                overlayZ === 'z-40' ? '' : 'hidden'
              } bg-[hsl(0_0%_10%)] ${overlayZ}`}
              onClick={handleOverlayClick}
            ></div>
          </>
        )}
              ...
              <div className="flex flex-row justify-center items-center gap-4">
                <div className="nav-sign-up">
                  <Button
                    ...
                    hover="hover:bg-redcoolerLite"
                    padding=" px-4 py-2"
                    outline="outline outline-1 outline-redcooler outline-offset-4 hover:outline-redcoolerLite"
                  />
                </div>
